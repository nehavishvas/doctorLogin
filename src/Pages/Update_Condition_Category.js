...
export const Update_Condition_Category = () => {
  const location = useLocation();
  const [form, setForm] = useState({
    category_name: "",
  });

  const { categoryName, _id } = location.state || {};

  useEffect(() => {
    if (location.state) {
      setForm((e) => ({ ...e, category_name: categoryName }));
    }
  }, [location.state]);
  ...
  const handleBookingAppointment = (e) => {
    ...
    if (validate()) {
      ...
      axios
        .put(`${medicalUrl}updateCategory/${_id}`, obj)
        .then((resp) => {
          Swal.fire({
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-8">
              <h5 className="addpatient">Update Condition Category</h5>
            </div>
          </div>
          <hr className="hrline" />
        </div>
