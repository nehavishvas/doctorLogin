...
export const Update_Course = () => {
  ...
  const { patientlist, doctorlist, getalltreatmentdata } =
  ...
  const { courseName, patientName, doctorName, _id, numberOfSession } =
    location.state || {};

  useEffect(() => {
    if (location.state) {
      console.log(location.state, "this is update course data");
      form.course_name = courseName;
      form.no_of_sessions = numberOfSession;
    }
  }, [location.state]);
  ...
  const handleBookingAppointment = (e) => {
    ...
    if (validate()) {
      ...
      axios
        .put(`${medicalUrl}updateTreatmentCourse/${_id}`, obj)
        .then((resp) => {
          console.log(resp);
          setForm({
            course_name: "",
