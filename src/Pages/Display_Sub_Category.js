...
export function Display_Sub_Category() {
  ...
  useEffect(() => {
    if (sub_category) {
      setRows(sub_category);
      setSearchApiData(sub_category);
    }
  }, [sub_category]);
  ...
  const handleFilter = (event) => {
    if (event.target.value === "") {
      ...
      const filterResult = searchApiData.filter((item) => {
        const fullName = `${item.subCategoryName}`.toLowerCase();
  ...
  const updateprofile = (row) => {
    navigate("./Update_Sub_Category", { state: row });
  };
  ...
  const deletestaff = (staffid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${medicalUrl}deleteSubCategory/${staffid}`)
          .then((resp) => {
            Swal.fire({
