...
export const Update_Staff = () => {
  ...
  useEffect(() => {
    if (location.state) {
      setForm({ ...row });
      console.log("this is update data coming from stafflsit", row);
      setImageUrl(`${medicalImageUrl}${row.profileImage}`);
    }
  }, [location.state]);
  ...
  const handleBookingAppointment = (e) => {
    ...
    if (validate()) {
      ...
      // Add selected pages to the form data using the proper format
      form.pages.forEach((page, index) => {
        formdata.append(`pages[${index}]`, page); // Use `page` directly as it represents the page title string
      });

      console.log("Form before submitting:", form);

      axios
        .put(`${medicalUrl}updateUserStaffData/${row._id}`, formdata)
        .then((resp) => {
          Swal.fire({
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-8">
              <h5 className="addpatient">Update Staff</h5>
            </div>
          </div>
          <hr className="hrline" />
        </div>
