...
export default function Patient_List() {
  ...
  const { todayappointment, APPOINTMENTDATA, patientlist } =
  ...
  useEffect(() => {
    if (todayappointment) {
      setRows(todayappointment);
      setSearchApiData(todayappointment);
    }

    if (patientlist) {
      setPatient(patientlist);
    }
  }, [todayappointment, patientlist]);
  ...
  const showalldetails = (e) => {
    Swal.fire({
      ...
      html: `
        ...
        <p><strong>Appointment Status:</strong> ${e.appointment_status}</p>
