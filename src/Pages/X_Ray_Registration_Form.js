...
export const X_Ray_Registration_Form = () => {
  const { XRAYFORMPASSDATA, xrayformpassdata } = useContext(AppContext);
  ...
  useEffect(() => {
    if (xrayformpassdata) {
      setAppointmentNo(xrayformpassdata.no);
      setPatientId(xrayformpassdata.id);
      setFormList((prevFormList) =>
        prevFormList.map((form) => ({
          ...form,
          appointmentno: xrayformpassdata.no,
          patientid: xrayformpassdata.id,
        }))
      );
    }
  }, [xrayformpassdata]);
  ...
  const handleBookingAppointment = (e) => {
    ...
    if (validate()) {
      XRAYFORMPASSDATA(formList);

      localStorage.setItem("xrayformpassdata", JSON.stringify(formList));
      console.log("formList", formList);

      navigate("X_Ray_Bill_Template");
    } else {
      console.log("please fill all the details in xray registration form");
    }
  };
  ...
  const handleChange = (index, e) => {
    ...
    setFormList((prevFormList) =>
      prevFormList.map((form, i) =>
        i === index
          ? { ...form, [name]: name === "xRayReport" ? files[0] : value }
          : form
      )
    );
  ...
  const addField = () => {
    ...
    setFormError((prevErrors) => [
      ...prevErrors,
      { testName: "", date: "", xRayReport: "", testFees: "", paymentMode: "" },
    ]);
  };
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-8">
              <h5 className="addpatient">X-Ray Registration Form</h5>
            </div>
          </div>
          <hr className="hrline" />
        </div>
