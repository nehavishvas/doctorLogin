...
export const X_Ray_Bill_Template = () => {
  const {
    getpatientapi,
    patientlist,
    dashboard,
  ...
  useEffect(() => {
    if (Array.isArray(xrayformpassdata) && xrayformpassdata.length > 0) {
      const formDataList = xrayformpassdata.map((form) => ({
        labTestName: form.testName,
        labTestFees: form.testFees,
        labTestDate: form.date,
        paymentMode: form.paymentMode,
        xRayReport: form.xRayReport,
      }));

      setPaymentMethod(xrayformpassdata[0].paymentMode);
      setFormdataList(formDataList);

      const total = formDataList.reduce(
        (sum, form) => sum + parseFloat(form.xrayTestFees),
        0
      );
      setTotalFees(total);

      const patientid = xrayformpassdata[0]?.patientid;
      if (patientid && Array.isArray(patientlist)) {
        const patientObj = patientlist.find((e) => e._id === patientid);
        setPatient(patientObj || {});
      }
    }
  }, [xrayformpassdata, patientlist]);
  ...
  const generatePDF = () => {
    ...
    Promise.all(requests)
      ...
      .then((canvas) => {
        ...
        pdf.save(`Lab_Bill_${invoiceno}.pdf`);
        ...
        >
          <div style={{ border: "1px solid #000", padding: "20px" }}>
            ...
            <p>
              ...
              {currentDate}
              <br />
              <strong>Invoice No.</strong> {xrayreportupload.length + 1}
              <br />
