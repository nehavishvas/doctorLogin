...
export function Treatment_Courses() {
  ...
  useEffect(() => {
    if (allTreatmentData) {
      setRows(allTreatmentData);
      setSearchApiData(allTreatmentData);
    }
  }, [allTreatmentData]);
  ...
  const displaysessions = (row) => {
    ...
    NUMBEROFSESSION(row.numberOfSession);
    console.log(
      "treatment course page h number of session",
      row.numberOfSession
    );
  ...
  const handleFilter = (event) => {
    if (event.target.value === "") {
      ...
      const filterResult = searchApiData.filter((item) => {
        const fullName = `${item.courseName}`.toLowerCase();
        const phone = `${item.patientName}`.toLowerCase();
  ...
  const updateprofile = (row) => {
    navigate("./Update_Course", { state: row });
  };
  ...
  const deletestaff = (staffid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${medicalUrl}deleteTreatmentCourse/${staffid}`)
          .then((resp) => {
            Swal.fire({
      ...
      <Paper sx={{ width: "100%", overflow: "hidden", padding: "12px" }}>
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>All Treatment Courses</div>
            <div>
              <NavLink to="Add_Course">
                  ...
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                      ...
                      <TableCell align="center">
                        {row.courseName ? row.courseName : "_"}
                      </TableCell>
