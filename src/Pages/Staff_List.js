...
export default function Staff_List() {
  ...
  const deletestaff = (staffid) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`${medicalUrl}deleteUserStaffData/${staffid}`)
          .then((resp) => {
            Swal.fire({
  ...
  const handleFilter = (event) => {
    if (event.target.value === "") {
      ...
      const filterResult = searchApiData.filter((item) => {
        ...
        const phone = `${item.role}`.toLowerCase();
  ...
  const handleSwitchChange = (id) => {
    axios
      .post(`${medicalUrl}updateUserStaffStatus/${id}`)
      .then((response) => {
        getstaffdata();
      })
      .catch((error) => {
      ...
      <Paper sx={{ width: "100%", overflow: "hidden", padding: "12px" }}>
        ...
        >
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div>All Staff List</div>
            <div>
              <NavLink to="Add_Staff">
                <button
                  ...
                  return (
                    <TableRow hover role="checkbox" tabIndex={-1} key={row._id}>
                      ...
                      <TableCell align="left">
                        <img
                          className="imgClass"
                          src={`${medicalImageUrl}${row.profileImage}`}
                        />
                      </TableCell>
